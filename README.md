...
### Reading Data in R
Make sure to include a copy of the `DataParser.R` file in your R project. Load the file into your own R code using `source(DataParser.R)`. To load your data into the R program, run the `get_temporal_doc_data(path, insert_na = TRUE)` function, inputting the output path you set when running the Rust program. The resulting object can be accessed directly or using one of the functions that take an object as input. Changing the default `insure_na = TRUE` input to `insert_na = FALSE` will prevent the program from inserting null values for time periods where no snapshots of a person's document were saved.
#### Importing Data
